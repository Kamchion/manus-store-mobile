...
# 'Descripcion' = valor de la variante (variantValue)
...
for row_idx in range(2, ws.max_row + 1):
    ...
    if not all([descripcion_idx, ciudad_idx, interior_idx, especial_idx]):
    ...
    if not valor_variante:
        skipped_count += 1
        continue
    
    if not all([precio_ciudad, precio_interior, precio_especial]):
        skipped_count += 1
        continue
    
    # Buscar la variante en la base de datos por variantValue
    cursor.execute("""
        SELECT pv.id, p.name
        FROM productVariants pv
        JOIN products p ON pv.productId = p.id
        WHERE pv.variantValue = %s
    """, (valor_variante,))
    ...
    if result:
        ...
        updated_count += 1
        print(f"✓ {product_name[:40]:40} - {valor_variante[:30]:30} | ${float(precio_ciudad):6.2f} / ${float(precio_interior):6.2f} / ${float(precio_especial):6.2f}")
    else:
        not_found_count += 1
...
print(f"Total omitidas (sin datos): {skipped_count}")
print(f"{'='*100}")
